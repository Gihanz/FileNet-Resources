...
set WASInstallerFullPath="%DirToDecompressTo%\%WASDirName%\WAS"
echo Installing IBM WebSphere Application Server 7.0 from %WASInstallerFullPath% ...

pushd "%WASInstallerFullPath%"
set INSTALLER-NAME=install.exe

if not exist %INSTALLER-NAME% goto EXIT_ERROR
call %INSTALLER-NAME% -options "%ScriptsDir%\installWAS70.txt" -silent
popd

time /t
:EXIT_OK
echo IBM WebSphere Application Server 7.0 installed
echo ====================
exit 0
