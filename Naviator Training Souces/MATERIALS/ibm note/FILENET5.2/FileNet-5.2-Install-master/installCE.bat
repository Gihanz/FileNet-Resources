...
set P8CEInstallerFullPath="%DirToDecompressTo%\%P8CEDirName%"
echo Installing IBM FileNet Content Engine from %P8CEInstallerFullPath% ...

pushd "%P8CEInstallerFullPath%"
set INSTALLER-NAME=5.2.0-P8CE-WIN.EXE

if not exist %INSTALLER-NAME% goto EXIT_ERROR

call %INSTALLER-NAME% -f "%ScriptsDir%\installCE.txt" -i silent
popd

time /t
:EXIT_OK
echo IBM FileNet Platform Content Engine installation complete
echo ====================
exit 0
