...
set P8PEInstallerFullPath="%DirToDecompressTo%\%P8PEFPDirName%"

echo Installing IBM FileNet Process Engine from %P8PEInstallerFullPath% ...

pushd "%P8PEInstallerFullPath%"
set INSTALLER-NAME=5.0.0.3-P8PE-WIN.EXE

if not exist %INSTALLER-NAME% goto EXIT_ERROR
call %INSTALLER-NAME% -f "%ScriptsDir%\installPEFP.txt"

popd

net stop VWServicesPEManager


copy /y "%ScriptsDir%\jpemgr.properties" "C:\program files\ibm\filenet\ProcessEngine\data\jpemgr.properties"
