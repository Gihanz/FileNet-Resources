...
pushd "%PBSTargetDrive%%PBSTargetRootDir64%\%P8NexusDirName%\configure"
echo "%PBSTargetDrive%%PBSTargetRootDir64%\%P8NexusDirName%\configure"
...
call %INSTALLER-NAME% execute -profile "%ScriptsDir%\profiles\NexusConfig"
popd
...
set PRO-PATH=%ScriptsDir%\profiles\NexusConfig\status

set F1=downloadcejarstask.ok
set F2=configurepe.ok
set F3=configurejdbcecm.ok
set F4=configureldap.ok
set F5=configureloginmodules.ok
set F6=importltpakey.ok
set F7=rebuildear.ok
set F8=deployapplication.ok

pushd %PRO-PATH%

for %%H in (%F1% %F2% %F3% %F4% %F5% %F6% %F7% %F8%) do if not exist %%H (
	echo ERROR - %%H not found. configNexus failed.  Refer to "%PRO-PATH%" for more details.
	exit -1	
)
