...
echo ====================
echo Start installPEClient.bat
time /t
...
set P8PEClientInstallerFullPath="%DirToDecompressTo%\%P8PEClientDirName%"
echo %P8PEClientInstallerFullPath%

echo Updating IBM FileNet Process Engine client files for Content Engine and Workplace XT ...

pushd "%P8PEClientInstallerFullPath%"
set INSTALLER-NAME=5.0.0.3-P8PE-CLIENT-WIN.EXE

if not exist %INSTALLER-NAME% goto EXIT_ERROR
call %INSTALLER-NAME% -f "%ScriptsDir%\installPEClient.txt" -i silent
popd

time /t
:EXIT_OK
echo  IBM FileNet Process Engine client files for Content Engine and Workplace XT updated
echo ====================
exit 0
