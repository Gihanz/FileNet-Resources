...
set P8XTInstallerFullPath="%DirToDecompressTo%\%P8XTDirName%"
echo Installing IBM FileNet Workplace XT from %P8XTInstallerFullPath% ...

pushd "%P8XTInstallerFullPath%"
set INSTALLER-NAME=WorkplaceXT-1.1.5.0-WIN.exe

if not exist %INSTALLER-NAME% goto EXIT_ERROR

call %INSTALLER-NAME% -options "%ScriptsDir%\installXT.txt" -silent
popd

copy "%ScriptsDir%\bootstrap.properties" "C:\Program Files\IBM\FileNet\Config\WebClient\bootstrap.properties"

copy "%ScriptsDir%\SitePreferences.xml" "C:\Program Files\IBM\FileNet\WebClient\WorkplaceXT\WEB-INF\xml\SitePreferences.xml"

time /t
:EXIT_OK
echo End installXT.bat
echo ====================
exit 0
